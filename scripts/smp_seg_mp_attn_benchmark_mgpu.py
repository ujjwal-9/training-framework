...
from torchmetrics.classification import BinaryConfusionMatrix, BinaryAUROC, BinaryJaccardIndex
from qtrain.dataset.infarct import InfarctDataset3D, InfarctDataset3D_Fast
...
parser.add_argument('--n_segmaps', default=1, type=int)
...
parser.add_argument('--cls_nclasses', default=2, type=int)
...
fabric = L.Fabric(accelerator="cuda", devices=8, strategy="ddp", precision="16-mixed")
fabric.launch()
...
train_dataloader = DataLoader(InfarctDataset3D_Fast(args_terminal), batch_size=args_terminal.batch_size, num_workers=args_terminal.num_workers, pin_memory=args_terminal.pin_memory, prefetch_factor=8)
if args_terminal.fast_dev_run == True:
    test_dataloader = DataLoader(InfarctDataset3D_Fast(args_terminal), batch_size=args_terminal.val_batch_size, num_workers=args_terminal.num_workers, pin_memory=args_terminal.pin_memory, prefetch_factor=8)
else:
    if args_terminal.valid_n_slices > 0:
        test_dataloader = DataLoader(InfarctDataset3D_Fast(args_terminal, 'valid'), batch_size=args_terminal.val_batch_size, shuffle=False, num_workers=args_terminal.num_workers, pin_memory=args_terminal.pin_memory, prefetch_factor=8)
    else:
        test_dataloader = DataLoader(InfarctDataset3D_Fast(args_terminal, 'valid'), batch_size=args_terminal.val_batch_size, shuffle=False, num_workers=args_terminal.num_workers, pin_memory=args_terminal.pin_memory, collate_fn=collate_fn, prefetch_factor=8)
...
miou = BinaryJaccardIndex(ignore_index=-100).to(device=device)

import segmentation_models_pytorch as smp

def seg_loss_criterion(pred, gt, trg_cls):  
    ...
    if torch.numel(pred_) == 0:
        return loss_dict, total_loss, None
    
    metric = miou((pred_>0.5).to(torch.int), gt_)
...
def slc_loss_criterion(pred, gt, trg_cls):
    NULL_SUM = torch.numel(torch.Tensor(gt.shape[1:]))*-100
    indices = torch.where((gt.sum(axis=(1,2,3)) == NULL_SUM) != True)[0]
...
def loss_criterion(pred, gt, trg_cls, prefix="train"):
    trg_cls = trg_cls[0]
    seg_loss_dict, seg_loss, metric = seg_loss_criterion(pred["masks"], gt[0], trg_cls)
    # cls_loss_dict, cls_loss, target = cls_loss_criterion(pred["cls_logits"], trg_cls)
    slc_loss_dict, slc_loss = slc_loss_criterion(pred["slc_logits"], gt, trg_cls)
...
for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        for iteration, (input_scan, targets, trg_cls) in enumerate(tqdm(train_dataloader, leave=True)):
            train_iter += 1
            iterPerEpoch += 1
            ...
            trg_cls = trg_cls.to(device=device)
            ...
            with fabric.no_backward_sync(model, enabled=is_accumulating):
                ...
                loss, log_losses, cls_target, metric = loss_criterion(output, targets, trg_cls)
        ...
        if (epoch+1) % 1 == 0:
            ...
            with torch.no_grad():
                for j, (input_scan, targets, trg_cls) in enumerate(tqdm(test_dataloader, leave=True)):
                    val_iter += 1
                    ...
                    if args_terminal.mp == True:
                        with torch.cuda.amp.autocast():
                            output = model(input_scan)
                            val_loss, log_losses, cls_target, metric = loss_criterion(output, targets, trg_cls, prefix="valid")                  
                    else:
                        output = model(input_scan)
                        val_loss, log_losses, cls_target, metric = loss_criterion(output, targets, trg_cls, prefix="valid")
