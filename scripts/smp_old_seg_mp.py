...
parser.add_argument('--pooling', default="softminmax", type=str)
...
parser.add_argument('--cls_loss_wts', default={"ce": 2.0}, type=json.loads)
parser.add_argument('--slc_loss_wts', default={"bce": 2.0}, type=json.loads)
...
from fresnet.model import FusionResnet
model = FusionResnet("seresnet50", attn_mode=args_terminal.pooling, img_size=(args_terminal.n_slices, args_terminal.img_size, args_terminal.img_size))
sd = torch.load("/home/users/ujjwal.upadhyay/packages/qureai/packages/python/qer/resources/checkpoints/infarcts/infarcts_model_epoch59_sens92_spec90_tsens95_tspec85.pth", map_location="cpu")
...
def loss_criterion(pred, gt, prefix="train"):
    seg_loss_dict, seg_loss, metric = seg_loss_criterion(pred[2], gt)
    cls_loss_dict, cls_loss, target = cls_loss_criterion(pred[0], gt)
    slc_loss_dict, slc_loss = slc_loss_criterion(pred[1].permute(0,2,1), gt)
...
for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        for i, (input_scan, targets) in enumerate(tqdm(train_dataloader, leave=True)):
            ...
            input_scan = input_scan.permute(0,2,1,3,4).to(device=device, dtype=torch.float, non_blocking=True)
            ...
            output_labels = torch.softmax(output[0], dim=1)
            ...
            if train_iter % log_every_n_steps == 0:
                ...
                writer.add_scalar('train/batch_miou_score', metric.item(), train_iter)
        ...
        writer.add_scalar('train/epoch_miou_score', train_avg_metric, train_iter)
            ...
            with torch.no_grad():
                for j, (input_scan, targets) in enumerate(tqdm(test_dataloader, leave=True)):
                    ...
                    if val_iter % log_every_n_steps == 0:
                        ...
                        writer.add_scalar('valid/batch_miou_score', metric.item(), val_iter)
                        for key in log_losses:
                ...
                writer.add_scalar('valid/epoch_miou_score', val_avg_metric, val_iter)
