...
class InfarctDataModule(pl.LightningDataModule):
    ...
    def setup_dataloader(self, dataset, batch_size):
        ...
        if self.args.sampler == True:
            ...
            sampler = WeightedRandomSampler(dataset.dataset.sample_wts.to_list(), self.args.n_samples)
...
class InfarctDataset3D(Dataset):
    ...
    def process(self, index):
        ...
        if self.args.n_slices > 0:
            if ct_scan.shape[0] < self.args.n_slices:
                ...
                annotation = torch.cat([annotation, torch.zeros(self.args.n_slices - annotation.shape[0], *annotation.shape[1:])])
    ...
    def train_transforms(self, input_scan, target):
        ...
        if random.random() < 0.7:
            ...
            target = TF.affine(target, angle=angle, translate=translate, scale=scale, shear=0.0, interpolation=transforms.InterpolationMode.BILINEAR)
...
#         with open(datapath, "r") as f:
        ...
        if self.run_type in ['train', 'test', 'valid']:
            ...
            self.max_classes = 0
            for index in tqdm(range(counter), desc=f"{self.run_type} data"):
                filepath, annotpath, labels, crop, cls = self.dataset.loc[index,"filepath"], self.dataset.loc[index,"annotpath"], self.dataset.loc[index,"labels"], self.dataset.loc[index, "crop"], self.dataset.loc[index, "annotation"]
                n_classes = len(labels)
                if n_classes > self.max_classes:
                    self.max_classes = n_classes
                if self.args.only_annotated:
...
#         return get_croped_3d_volume(scan, annot, [self.args.n_slices]+self.input_shape)
    ...
    def get_foreground_crop(self, sitk_arr, annot_arr, index):
        ...
        cropped_sitk_arr = sitk_arr[:, coord_top[0]:coord_bottom[0], coord_top[1]:coord_bottom[1]]
        cropped_annot_arr = annot_arr[:, coord_top[0]:coord_bottom[0], coord_top[1]:coord_bottom[1]]
        return cropped_sitk_arr, cropped_annot_arr
    
    def process(self, index):
        annotation = self.process_annotation_label(index)
        if self.datapath[index].split(".")[-1] == "safetensors" or self.datapath[index].split(".")[-1] == "safetensor":
...
#         ct_scan, annotation = self.initialize_transform()(ct_scan), self.initialize_transform(True)(annotation)
        ...
        if self.args.n_slices > 0:
            if ct_scan.shape[0] < self.args.n_slices:
                ct_scan = torch.cat([ct_scan, torch.zeros(self.args.n_slices - ct_scan.shape[0], *ct_scan.shape[1:]).to(torch.int16)])
                if is_ignore_index:
                    annotation = torch.cat([annotation, torch.ones(self.args.n_slices - annotation.shape[0], *annotation.shape[1:]).to(torch.int16)*-100])
                else:
                    annotation = torch.cat([annotation, torch.zeros(self.args.n_slices - annotation.shape[0], *annotation.shape[1:]).to(torch.int16)])
            elif ct_scan.shape[0] > self.args.n_slices:
                start_slice = np.random.randint(0, ct_scan.shape[0] - self.args.n_slices)
                ct_scan = ct_scan[start_slice:start_slice+self.args.n_slices]
                annotation = annotation[start_slice:start_slice+self.args.n_slices]
...
#         ct_scan = ct_scan.permute(3,0,1,2)
    ...
    def __getitem__(self, index):
        try:
            ...
            return *self.excute_augmentations(ct_scan, annotation), torch.Tensor([label_class])
        except:
    ...
    def train_transforms(self, input_scan, target):
        ...
        target = transforms.Resize(self.input_shape, transforms.InterpolationMode.NEAREST)(target)
        if self.run_type == 'train' and self.args.augmentation:
            state = torch.get_rng_state()
            if random.random() < 0.7:
                translate = [random.uniform(-10, 10), random.uniform(-10, 10)]
...
class InfarctDataset3D_60k(Dataset):
    ...
    def append_dataset(self, scan, annotation, label, crop, cls, acute, chronic, normal, series):
        self.datapath.append(scan)
        self.targetpath.append(annotation)
        self.labels.append(label)
...
#         with open(datapath, "r") as f:
        ...
        if self.run_type in ['train', 'test', 'valid']:
            ...
            for index in tqdm(range(counter), desc=f"{self.run_type} data"):
                ...
                filepath, annotpath, labels, crop, cls = self.dataset.loc[index,"filepath"], \
                ...
                acute, chronic, normal = self.dataset.loc[index,"acute"],\
                ...
                if self.args.only_annotated:
                    ...
                    for key in labels:
                        if n_classes > 0:
                            if_annotated = True
                    if if_annotated:
                        self.append_dataset(filepath, annotpath, labels, crop, cls, acute, chronic, normal, series)
                elif self.args.only_annotated == False:
                    self.append_dataset(filepath, annotpath, labels, crop, cls, acute, chronic, normal, series)
        if self.args.mode == "multiclass":
            self.max_classes += 1
...
#         ct_scan, annotation = self.initialize_transform()(ct_scan), self.initialize_transform(True)(annotation)
        ...
        ct_scan = self.get_window_channels(ct_scan.to(torch.float))
        return ct_scan, annotation, [self.normal[index]], [self.acute[index], self.chronic[index]]
...
class InfarctDataset3D_only_cls(Dataset):
    ...
    def process(self, index):
        ...
        if self.datapath[index].split(".")[-1] == "safetensor" or self.datapath[index].split(".")[-1] == "safetensors":
            with safe_open(self.datapath[index], framework="pt", device="cpu") as f:
                for k in f.keys():
                    ct_scan = f.get_tensor(k).numpy()
        elif self.datapath[index].split(".")[-1] == "h5":
