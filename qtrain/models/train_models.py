...
class qMultiTasker(pl.LightningModule):
    def __init__(self, args):
        ...
        self.seg_focal_loss = smp.losses.FocalLoss(
            mode="multiclass", ignore_index=self.ignore_index, reduction="none"
        )
        ...
        self.cls_ce_loss = nn.CrossEntropyLoss(
            weight=torch.FloatTensor([2.0, 1.0]), ignore_index=self.ignore_index
        )
        self.cls_auc_metric = AUROC(
            task="binary", num_classes=2, ignore_index=self.ignore_index
        )
        # self.cls_ap_metric = tm.AveragePrecision(task="binary", num_classes=2, ignore_index=self.ignore_index)
        self.cls_sens_spec_metric = tm.StatScores(
            task="binary", num_classes=2, ignore_index=self.ignore_index
        )

        self.slc_bce_loss = nn.BCEWithLogitsLoss(reduction="none")
        self.slc_auc_metric = AUROC(
            task="multiclass", num_classes=2, ignore_index=self.ignore_index
        )
        # self.slc_ap_metric = tm.AveragePrecision(task="multiclass", num_classes=2, ignore_index=self.ignore_index)
        self.slc_sens_spec_metric = tm.StatScores(
            task="multiclass", num_classes=2, ignore_index=self.ignore_index
        )
        ...
        self.infarct_sens_spec_metric = tm.StatScores(
            task="multilabel",
            num_labels=2,
            ignore_index=self.ignore_index,
            average="none",
        )
        self.infarct_auc_metric = AUROC(
            task="binary", num_classes=2, ignore_index=self.ignore_index
        )
    ...
    def seg_loss_criterion(self, pred, gt, series, prefix):
        ...
        for key in self.args.seg_loss_wts:
            if torch.all(gt == self.ignore_index):
                loss_dict[key] = self.args.seg_loss_wts[key] * seg_losses[key](
        ...
        if prefix == "train":
            self.train_epoch_iou_storage.extend(
                self.seg_miou_metric(
                    pred_.view(-1, *pred_.size()[2:]), gt.view(-1, *gt.size()[2:])
                ).tolist()
            )
        else:
            self.epoch_iou_storage.extend(
                self.seg_miou_metric(
                    pred_.view(-1, *pred_.size()[2:]), gt.view(-1, *gt.size()[2:])
                ).tolist()
            )
    ...
    def cls_loss_criterion(self, pred, gt, series, prefix):
        ...
        for key in self.args.cls_loss_wts:
            loss_dict[key] = self.args.cls_loss_wts[key] * cls_losses[key](
    ...
    def slc_loss_criterion(self, pred, gt, series, prefix):
        ...
        for key in self.args.slc_loss_wts:
            loss_dict[key] = (
                self.args.slc_loss_wts[key]
                * slc_losses[key](pred[:, :, 1].double(), target_score.double())
                * (target_score >= 0)
            )
            loss_dict[key] = torch.mean(loss_dict[key])
            total_loss += loss_dict[key]
    ...
    def infarct_loss_criterion(self, pred, target_score, serie, prefix):
        ...
        for key in self.args.slc_loss_wts:
            loss_dict[key] = (
                self.args.ac_loss_wts[key]
                * slc_losses[key](pred.double(), target_score.double())
                * (target_score >= 0)
            )
            loss_dict[key] = torch.mean(loss_dict[key])
            total_loss += loss_dict[key]
    ...
    def compute_epoch_metric(self, prefix):
        if prefix == "train":
            ...
            epoch_metric[f"{prefix}_seg_epoch_miou"] = torch.mean(
                torch.tensor(self.train_epoch_iou_storage)
            )

            epoch_metric[f"{prefix}_slc_epoch_auc"] = self.slc_auc_metric(
                torch.vstack(self.train_slc_scores["scores"]),
                torch.vstack(self.train_slc_scores["gt"]),
            )
            epoch_metric[f"{prefix}_slc_epoch_sensitivity"] = (
                self.train_slc_confusion_matrix["tp"] + 1e-6
            ) / (
                self.train_slc_confusion_matrix["tp"]
                + self.train_slc_confusion_matrix["fn"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_slc_epoch_specificity"] = (
                self.train_slc_confusion_matrix["tn"] + 1e-6
            ) / (
                self.train_slc_confusion_matrix["tn"]
                + self.train_slc_confusion_matrix["fp"]
                + 1e-6
            )
            ...
            epoch_metric[f"{prefix}_infarct_epoch_acute_auc"] = self.infarct_auc_metric(
                torch.vstack(self.train_acute_scores["scores"]),
                torch.vstack(self.train_acute_scores["gt"]),
            )
            epoch_metric[f"{prefix}_infarct_epoch_acute_sensitivity"] = (
                self.train_acute_confusion_matrix["tp"] + 1e-6
            ) / (
                self.train_acute_confusion_matrix["tp"]
                + self.train_acute_confusion_matrix["fn"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_infarct_epoch_acute_specificity"] = (
                self.train_acute_confusion_matrix["tn"] + 1e-6
            ) / (
                self.train_acute_confusion_matrix["tn"]
                + self.train_acute_confusion_matrix["fp"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_infarct_epoch_acute_youden"] = (
                epoch_metric[f"{prefix}_infarct_epoch_acute_sensitivity"]
                + epoch_metric[f"{prefix}_infarct_epoch_acute_specificity"]
                - 1
            )
            ...
            epoch_metric[
                f"{prefix}_infarct_epoch_chronic_auc"
            ] = self.infarct_auc_metric(
                torch.vstack(self.train_chronic_scores["scores"]),
                torch.vstack(self.train_chronic_scores["gt"]),
            )
            epoch_metric[f"{prefix}_infarct_epoch_chronic_sensitivity"] = (
                self.train_chronic_confusion_matrix["tp"] + 1e-6
            ) / (
                self.train_chronic_confusion_matrix["tp"]
                + self.train_chronic_confusion_matrix["fn"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_infarct_epoch_chronic_specificity"] = (
                self.train_chronic_confusion_matrix["tn"] + 1e-6
            ) / (
                self.train_chronic_confusion_matrix["tn"]
                + self.train_chronic_confusion_matrix["fp"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_infarct_epoch_chronic_youden"] = (
                epoch_metric[f"{prefix}_infarct_epoch_chronic_sensitivity"]
                + epoch_metric[f"{prefix}_infarct_epoch_chronic_specificity"]
                - 1
            )
            ...
            epoch_metric = defaultdict()
            epoch_metric[f"{prefix}_seg_epoch_miou"] = torch.mean(
                torch.tensor(self.epoch_iou_storage)
            )

            epoch_metric[f"{prefix}_slc_epoch_auc"] = self.slc_auc_metric(
                torch.vstack(self.slc_scores["scores"]),
                torch.vstack(self.slc_scores["gt"]),
            )
            epoch_metric[f"{prefix}_slc_epoch_sensitivity"] = (
                self.slc_confusion_matrix["tp"] + 1e-6
            ) / (
                self.slc_confusion_matrix["tp"] + self.slc_confusion_matrix["fn"] + 1e-6
            )
            epoch_metric[f"{prefix}_slc_epoch_specificity"] = (
                self.slc_confusion_matrix["tn"] + 1e-6
            ) / (
                self.slc_confusion_matrix["tn"] + self.slc_confusion_matrix["fp"] + 1e-6
            )
            epoch_metric[f"{prefix}_slc_epoch_youden"] = (
                epoch_metric[f"{prefix}_slc_epoch_sensitivity"]
                + epoch_metric[f"{prefix}_slc_epoch_specificity"]
            ...
            epoch_metric[f"{prefix}_infarct_epoch_acute_auc"] = self.infarct_auc_metric(
                torch.vstack(self.acute_scores["scores"]),
                torch.vstack(self.acute_scores["gt"]),
            )
            epoch_metric[f"{prefix}_infarct_epoch_acute_sensitivity"] = (
                self.acute_confusion_matrix["tp"] + 1e-6
            ) / (
                self.acute_confusion_matrix["tp"]
                + self.acute_confusion_matrix["fn"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_infarct_epoch_acute_specificity"] = (
                self.acute_confusion_matrix["tn"] + 1e-6
            ) / (
                self.acute_confusion_matrix["tn"]
                + self.acute_confusion_matrix["fp"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_infarct_epoch_acute_youden"] = (
                epoch_metric[f"{prefix}_infarct_epoch_acute_sensitivity"]
            ...
            epoch_metric[
                ...
                torch.vstack(self.chronic_scores["scores"]),
                torch.vstack(self.chronic_scores["gt"]),
            )
            epoch_metric[f"{prefix}_infarct_epoch_chronic_sensitivity"] = (
                self.chronic_confusion_matrix["tp"] + 1e-6
            ) / (
                self.chronic_confusion_matrix["tp"]
                + self.chronic_confusion_matrix["fn"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_infarct_epoch_chronic_specificity"] = (
                self.chronic_confusion_matrix["tn"] + 1e-6
            ) / (
                self.chronic_confusion_matrix["tn"]
                + self.chronic_confusion_matrix["fp"]
                + 1e-6
            )
            epoch_metric[f"{prefix}_infarct_epoch_chronic_youden"] = (
                epoch_metric[f"{prefix}_infarct_epoch_chronic_sensitivity"]
...
class qSegAndClass(pl.LightningModule):
    def __init__(self, args):
        ...
        self.seg_miou_metric = JaccardIndex(
            task="binary", num_classes=2, ignore_index=-100
        )
        self.seg_auc_metric = AUROC(task="binary", num_classes=2, ignore_index=-100)
        self.seg_ap_metric = tm.AveragePrecision(
            task="binary", num_classes=2, ignore_index=-100
        )

        self.cls_ce_loss = nn.CrossEntropyLoss()
        self.cls_auc_metric = AUROC(task="binary", num_classes=2, ignore_index=-100)
        self.cls_ap_metric = tm.AveragePrecision(
            task="binary", num_classes=2, ignore_index=-100
        )
        self.cls_sens_spec_metric = tm.StatScores(
            task="binary", num_classes=2, ignore_index=-100
        ...
        self.slc_ap_metric = tm.AveragePrecision(
            task="multiclass", num_classes=2, ignore_index=-100
        )
        self.slc_sens_spec_metric = tm.StatScores(
            task="multiclass", num_classes=2, ignore_index=-100
        )
    ...
    def seg_loss_criterion(self, pred, gt):
        ...
        seg_metric = {
            "miou": self.seg_miou_metric(F.softmax(pred).argmax(2).detach(), gt),
    ...
    def slc_loss_criterion(self, pred, gt):
        ...
        loss_dict = {}
        for key in self.args.slc_loss_wts:
            loss_dict[key] = self.args.slc_loss_wts[key] * slc_losses[key](
                pred[:, :, 1][target_score >= 0].double(),
                target_score[target_score >= 0].double(),
            )
            if torch.isnan(loss_dict[key]):
                # loss_dict[key] = torch.tensor(self.nan_score, device=self.device, requires_grad=True)
                loss_dict[key][torch.isnan(loss_dict[key])] = self.nan_score
        ...
        tp, fp, tn, fn, sup = self.slc_sens_spec_metric(
            F.softmax(pred).detach().permute(0, 2, 1), target_score.detach()
        )
        slc_metric = {
    ...
    def loss_criterion(self, pred, gt, trg_cls, prefix="train"):
        ...
        for loss_ in losses_:
            if loss_ is not None:
                loss += loss_
        losses = {"seg": seg_loss_dict, "slc": slc_loss_dict, "cls": cls_loss_dict}

        metrics = {"seg": seg_metric, "slc": slc_metric, "cls": cls_metric}
        ...
        for key in metrics:
            if metrics[key] is None:
                ...
                for l in metrics[key]:
                    if torch.isnan(metrics[key][l]):
                        metrics[key][l] = torch.tensor(
                            self.nan_score, device=self.device
                        )
    ...
    def compute_batch(self, batch, batch_idx, prefix="train"):
        ...
        output = self(ct)
        loss, log_losses, log_metrics = self.loss_criterion(
            output, gt_segmentation_map, trg_cls, prefix
        )
        return loss, log_losses, log_metrics
...
class qSegmentation(pl.LightningModule):
    ...
    def cls_loss_criterion(pred, gt):
        ...
        for key in args_terminal.cls_loss_wts:
            loss_dict[key] = args_terminal.cls_loss_wts[key] * cls_losses[key](
                pred, target
            )
            total_loss += loss_dict[key]
    ...
    def slc_loss_criterion(pred, gt):
        ...
        for key in args_terminal.slc_loss_wts:
            loss_dict[key] = args_terminal.slc_loss_wts[key] * slc_losses[key](
                pred[:, :, 1], target_score
            )
            total_loss += loss_dict[key]
    ...
    def loss_fn(self, model_output, gt_segmentation_map, prefix="train/"):
        ...
        for i, loss in enumerate(self.losses):
            computed_losses[prefix + loss] = self.args.loss_contrib[loss] * self.losses[
                loss
            ](model_output, gt_segmentation_map)
...
class qClassification(pl.LightningModule):
    ...
    def process_batch_for_epoch(self, batch_numbers, prefix="train/"):
        ...
        for loss in self.losses:
            epoch_end[epoch_prefix + loss] = torch.stack(
                batch_numbers["loss"][batch_prefix + loss]
            ).mean()
        for metric in self.metrics:
            epoch_end[epoch_prefix + metric] = torch.stack(
                batch_numbers["metric"][batch_prefix + metric]
            ).mean()
